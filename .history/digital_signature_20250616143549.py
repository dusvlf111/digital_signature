...
class DrawingCanvas:
    def __init__(self, parent):
        self.parent = parent
        self.canvas = tk.Canvas(parent, width=500, height=250, bg='white', 
        ...
        ttk.Button(control_frame, text="지우기", command=self.clear_canvas).pack(side=tk.LEFT, padx=10)
        ttk.Button(control_frame, text="색상 변경", command=self.choose_color).pack(side=tk.LEFT, padx=10)
        
        # 펜 굵기 조절
        ttk.Label(control_frame, text="펜 굵기:").pack(side=tk.LEFT, padx=(20, 5))
        self.width_var = tk.IntVar(value=3)
        width_scale = ttk.Scale(control_frame, from_=1, to=15, variable=self.width_var, 
                               orient=tk.HORIZONTAL, length=150)
        width_scale.pack(side=tk.LEFT, padx=5)
        width_scale.bind('<Motion>', self.update_pen_width)
        
        # 현재 설정 표시
        self.info_label = ttk.Label(control_frame, text=f"색상: {self.pen_color} | 굵기: {self.pen_width}")
...
class HandwrittenSignatureApp:
    ...
    def setup_ui(self):
        ...
        # 그리기 캔버스
        canvas_frame = ttk.LabelFrame(main_frame, text="서명 그리기 영역", padding="10")
        ...
        ttk.Button(button_frame, text="미리보기", 
                  command=self.preview_signature).pack(side=tk.LEFT, padx=10)
        
        ttk.Button(button_frame, text="이미지 저장", 
                  command=self.save_image).pack(side=tk.LEFT, padx=10)
    ...
    def show_preview(self, img):
        ...
        preview_img.thumbnail((400, 150), Image.Resampling.LANCZOS)
