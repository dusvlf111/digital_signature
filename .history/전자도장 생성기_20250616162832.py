    ...
    def create_stamp_image(self):
        ...
        for line in self.drawn_lines:
            ...
            if self.carving_var.get() == "양각":
                line_color = self.lighten_color(final_color, 0.5)
            elif self.carving_var.get() == "음각":
                line_color = self.darken_color(final_color, 0.4)
    ...
    def apply_carving_effect(self, img):
        ...
        try:
            if self.carving_var.get() == "양각":
                ...
                for y in range(img.height):
                    for x in range(img.width):
                        if pixels[x, y][3] > 0:  # 투명하지 않은 픽셀만
                            r, g, b, a = pixels[x, y]
                            # 하이라이트: 더 밝게
                            highlight_r = min(255, int(r * 1.4))
                            highlight_g = min(255, int(g * 1.4))
                            highlight_b = min(255, int(b * 1.4))
                            pixels[x, y] = (highlight_r, highlight_g, highlight_b, a)
                            
                            # 그림자: 더 어둡게
                            shadow_r = max(0, int(r * 0.6))
                            shadow_g = max(0, int(g * 0.6))
                            shadow_b = max(0, int(b * 0.6))
                ...
                # 하이라이트를 왼쪽 위로 이동
                final_img.paste(highlight_img, (-1, -1), highlight_img)
                # 원본을 중앙에
                ...
                for y in range(img.height):
                    for x in range(img.width):
                        if pixels[x, y][3] > 0:  # 투명하지 않은 픽셀만
                            ...
                            dark_g = max(0, int(g * 0.5))
                            dark_b = max(0, int(b * 0.5))
                            pixels[x, y] = (dark_r, dark_g, dark_b, a)
                
                # 블러 효과 추가
                blurred = dark_img.filter(ImageFilter.GaussianBlur(radius=1))
                
                # 최종 이미지 합성
                final_img = Image.new('RGBA', img.size, (255, 255, 255, 0))
                final_img.paste(blurred, (1, 1), blurred)
                final_img.paste(dark_img, (0, 0), dark_img)
                return final_img
                
        except Exception as e:
            print(f"조각 효과 적용 중 오류: {e}")
