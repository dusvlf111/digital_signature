...
class DigitalSignatureGenerator:
            ...
            if os.path.exists(font_dir):
                ...
                for file in font_files:
                    if file.lower().endswith(('.ttf', '.otf')):
                        ...
                        font_paths.append((font_name, os.path.join(font_dir, file)))
                ...
                if os.path.exists(font_dir):
                    for root, dirs, files in os.walk(font_dir):
                        for file in files:
                            if file.lower().endswith(('.ttf', '.otf')):
                                font_name = os.path.splitext(file)[0]
                                font_paths.append((font_name, os.path.join(root, file)))
        
        # 폰트 이름만 추출하고 중복 제거
        font_names = list(set([name for name, path in font_paths]))
        
        # 기본 안전한 폰트들 추가
        safe_fonts = ["Default", "Arial", "Times New Roman", "Courier New"]
        
        # 최종 폰트 목록
        all_fonts = safe_fonts + sorted(font_names)
    ...
    def get_font(self, font_name, size):
        ...
        if os.name == 'nt':  # Windows
            ...
            if font_name in windows_fonts:
                font_paths.append(f'C:/Windows/Fonts/{windows_fonts[font_name]}')
            ...
            font_paths.extend([
                ...
                'C:/Windows/Fonts/arial.ttf',  # 최종 fallback
            ])
            ...
            font_paths.extend([
                f'/usr/share/fonts/truetype/dejavu/DejaVu{font_name.replace(" ", "")}.ttf',
                f'/usr/share/fonts/truetype/liberation/Liberation{font_name.replace(" ", "")}-Regular.ttf',
    ...
    def test_font(self):
        ...
        try:
            ...
            test_text = "폰트 테스트 Font Test 123"
            draw.text((10, 30), test_text, font=test_font, fill='black')
            
            self.current_image = img
            self.show_preview(img)
            messagebox.showinfo("폰트 테스트", f"'{font_name}' 폰트가 정상적으로 로드되었습니다.")
